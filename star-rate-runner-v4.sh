...
--argument \"rand($LOWRANGE,$HIGHRANGE)\" \"(uid=user.{1})\""
  ...
  echo -n "Waiting for operations to complete..."
  for pid in ${pids[*]}; do
    wait $pid
    echo -n "."
  done
...
for host in $HOSTNAME; do
  for optype in $OPTYPES; do
    firstrun=true
    for file in $(ls $OUTPUTHOME/$optype-*-$host.csv); do
      if $firstrun; then
    ...
    cat $OUTPUTHOME/$host-$optype-all.csv.tmp | sed 's/$/,/' >$OUTPUTHOME/$host-$optype-all.csv
    rm $OUTPUTHOME/$host-$optype-all.csv.tmp
    echo -n "."
  done
done
